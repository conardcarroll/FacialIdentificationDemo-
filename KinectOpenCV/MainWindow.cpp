...
int CMainWindow::Run(HINSTANCE hInstance, int nCmdShow)
{
	...
	m_openCVFaceRecog.TrainModel( "C:/Tvision-insights-master/PythonScript/att.txt") ; // time consuming, need to serialize 
    ...
    while (continueProcessing)
    {
        ...
        if (m_frameHelper.IsInitialized()) 
        {
            ...
            if (!m_bIsColorPaused && SUCCEEDED(m_frameHelper.UpdateColorFrame())) 
            {
				...
				m_res = m_openCVFaceRecog.Prediction(&dst) ;
...
{   
    ...
    WaitForSingleObject(m_hColorResolutionMutex, INFINITE);
    wstring colorStreamInfoText = GenerateStreamInformation(m_colorResolution, m_colorFilterID, m_colorFrameRateTracker.CurrentFPS(), m_res);
    ReleaseMutex(m_hColorResolutionMutex);
    ...
    WaitForSingleObject(m_hDepthResolutionMutex, INFINITE);
    wstring depthStreamInfoText = GenerateStreamInformation(m_depthResolution, m_depthFilterID, m_depthFrameRateTracker.CurrentFPS(), m_res);
    ReleaseMutex(m_hDepthResolutionMutex);
...
wstring CMainWindow::GetFaceID(int faceID)
{
    wostringstream stream;
    stream << faceID;
    return _TEXT("Person ID: ") + stream.str();
}
...
wstring CMainWindow::GenerateStreamInformation(NUI_IMAGE_RESOLUTION resolution, int filterID, double frameRate, int faceID)
{
